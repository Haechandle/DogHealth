...
4. 숙면여부 측정 -자는지 체크하는 함수, 자고있을때 수행하는 함수
5. 측정중 움직였을때 수면상태인지 자다가 뒤척인 상태인지 측정하는 코드를 함수로 묶고 숙면여부 측정에 활용
등등

 *  #06.10# 수정사항
1. 자체적으로 수면의 질을 판단하는게 아니고 디비에 추가로 수면시간(초단위)이랑 일어난 횟수를 보낸다.
test 결과: 정상작동
 ...
 *  기타사항
잠에서 깨어난 후에 바로 timecount()로 진입하여 수면정보값을 바로 못보내줌.. 따라서 깨어나 있을 경우엔 착용을 해제하는 것을 전제로 해야 될듯. 
...
const int temp_whileSleeping=5; //온도. 온도로도 자는것을 구분할 수 있는지 확인하고 사용. 아직 사용은 x
...
void loop() {
    ...
    //int strolled= checkifStroll(); //산책 여부 값 저장
      ...
      while(really_Sleeping>3){
            ...
            Serial.print("Time: "); Serial.print(sleepTime);
            Serial.print("     Awaken count: "); Serial.println(wakeup_Count);
                       
            Serial.print("Count none sleeping heartRate: "); Serial.println(really_Sleeping);
            
            sleepTime+=term_to_second; //총 수면시간 측정
            delay(1000);
            ...
            if(moved==-1){
                ...
                phpHost+="/input.php?temp=" + String(temp) + "&heart=" + String(heart) + "&goodOrbad=" + String(goodOrbad) + "&sleepTime=" + String(sleepTime) + "&wakeup_Count=" + String(wakeup_Count);
     ...
     while(heart>=abnormal_heart_low&&heart<=abnormal_heart_high&&temp>=abnormal_temp_low&&temp<=abnormal_temp_high){
        ...
        phpHost+="/input.php?temp=" + String(temp) + "&heart=" + String(heart) + "&goodOrbad=" + "0" + "&sleepTime=" + "0" + "&wakeup_Count=" + "0";
